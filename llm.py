...
class LLM:
        ...
        if use_cache:
            try:
                ...
                self.cache = pickle.load(
                    open(f"tmp/cache-{name.split('/')[-1]}.p", "rb")
                )
            except:
                self.cache = {}
        else:
    ...
    def __call__(
        ...
        cache_key = (
            ...
            else tuple(conversation + [add_image.tobytes()])
        )
        ...
        for _ in range(3):
            try:
                ...
                def request_with_timeout():
                    return self.model.make_request(
                        conversation,
                        add_image=add_image,
        ...
        if self.use_cache and response != "Model API request failed":
            ...
            pickle.dump(
                self.cache, open(f"tmp/cache-{self.name.split('/')[-1]}.p", "wb")
            )
