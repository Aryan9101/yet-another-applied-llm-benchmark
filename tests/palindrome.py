...
1. Create a list of exactly 100 different strings, including both palindromes and non-palindromes. Don't resort to hardcoding strings.
...
- Prints the contents of the csv file as a string to the consoleand ensure that's the only console output
'''
...
def check_correctness(csv):
    ...
    if len(df) != 100:
        assert False, "The csv file must contain exactly 100 data points"
    
    # Check if the csv file has the correct columns
    if not all(col in df.columns for col in ['text', 'is_palindrome']):
        return False, "The csv file must contain the columns 'text' and 'is_palindrome'"
    
    for index, row in df.iterrows():
        if row['is_palindrome'] not in [0, 1]:
            return False, "The is_palindrome column must contain only 0 or 1"
